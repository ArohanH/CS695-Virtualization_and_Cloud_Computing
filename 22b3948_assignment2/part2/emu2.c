...
//Helper Function
int run_one_guest(struct vm *vm)
{
    int ret = ioctl(vm->vcpus->vcpu_fd, KVM_RUN, 0);
    if (ret < 0) {
        perror("KVM_RUN failed");
        exit(1);
    }

    struct kvm_run *run = vm->vcpus->kvm_run; 
    int exit_reason = run->exit_reason;

    //printf("VMFD: %d stopped running - exit reason: %d\n",
    //       vm->vm_fd, exit_reason);

    switch (exit_reason) {
    case KVM_EXIT_IO:
    {
        int port = run->io.port;
        int *data_ptr = (int *)((char*)run + run->io.data_offset);
        int value = *data_ptr;
        ...
        switch (port) {
        case 0x10:
            shared_value[counter] = value;
            have_data[counter] = 1;  
            counter=(counter+1)%3;
            printf("VMFD: %d Produced value: %d\n", vm->vm_fd, value);
            break;
        case 0x11:
            if (have_data[counter]) {
                *data_ptr = shared_value[counter];
            } else {
                *data_ptr = 0;
            }
            break;
    ...
    case KVM_EXIT_DEBUG:
    default:
        printf("VMFD: %d KVM PANIC, reason=%d\n", vm->vm_fd, exit_reason);
        return -1; 
    }
...
{
    //printf("Starting emu with one physical CPU\n");
    ...
    {
        //printf("Running VM1...\n");
        if (run_one_guest(vm1) < 0) {
            printf("VM1 asked to shutdown or panicked\n");
            break;
        }
        ...
        //printf("Running VM2...\n");
        if (run_one_guest(vm2) < 0) {
            printf("VM2 asked to shutdown or panicked\n");
            break;
        }
        //printf("Running VM2...\n");
        if (run_one_guest(vm2) < 0) {
...
exit_kvm:
    return 0;
    // {Remove everything in the function above this line and replace it with your code here} - originally there
}
