...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	uint64_t memval = 0;
	uint64_t numExits = 0;
	uint64_t numExitsByIO_in = 0;
	uint64_t numExitsByIO_out = 0;
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			else if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xEB)
			{
				...
				numExits++;
				numExitsByIO_in++;
				*(uint32_t *) (p + vcpu->kvm_run->io.data_offset)=numExits;
				continue;
			}
			else if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEC)
			{
				...
				//printf("HC_printStr\n");
				char *p = (char *)vcpu->kvm_run;
				uintptr_t guest_ptr = *(uintptr_t *)(p + vcpu->kvm_run->io.data_offset);
				char *addr = (char *) guest_ptr;
				struct kvm_translation translation;
				translation.linear_address = (uint64_t)addr;
				if (ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &translation) == -1) {
				...
				if (!translation.valid) {
					...
					char* str = &vm->mem[translation.physical_address];
					printf("%s", str);
					fflush(stdout);
				}
				numExits++;
			...
			{
				//HC_numExitsByType
				//printf("HC_numExitsByType\n");
				numExits++;
				numExitsByIO_in++;
				char *p = (char *)vcpu->kvm_run;
				char buffer[64];
    			sprintf(buffer, "IO in: %lu\nIO out: %lu\n",numExitsByIO_in, numExitsByIO_out);
				uint32_t offset = 0x20000+ (uint32_t) numExitsByIO_in * 64; //random GVA for the guest 
				struct kvm_translation translation;
				translation.linear_address = (uint64_t) offset;
				if (ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &translation) == -1) {
					perror("ioctl KVM_TRANSLATE failed");
					exit(EXIT_FAILURE);
				}
				if (!translation.valid) {
					printf("Invalid Guest Virtual Address: 0x%x\n", offset);
					fflush(stdout);
				}
				else{
					//printf("Guest Virtual Address: 0x%x\n", offset);
					//printf("Host Virtual Address: 0x%llx\n", translation.physical_address);
					strcpy((char *)&vm->mem[translation.physical_address], buffer);
					*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = offset;
					fflush(stdout);
				}
				continue;
			}
			else if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEE)
			{
				...
				//printf("HC_gvaToHva\n");
				char *p = (char *)vcpu->kvm_run;
				uint32_t guest_ptr_to_mem = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				struct kvm_translation translation1;
				translation1.linear_address = (uint64_t) guest_ptr_to_mem;
				if (ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &translation1) == -1) {
					perror("ioctl KVM_TRANSLATE failed (translation1)");
					exit(EXIT_FAILURE);
				}
				if (!translation1.valid) {
					printf("Invalid guest pointer 0x%x\n", guest_ptr_to_mem);
					exit(EXIT_FAILURE);
				}
				uint32_t gva_value = *(uint32_t *)((char *)vm->mem + translation1.physical_address);
				struct kvm_translation translation2;
				translation2.linear_address = (uint64_t) gva_value;
				if (ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &translation2) == -1) {
					perror("ioctl KVM_TRANSLATE failed (translation2)");
					exit(EXIT_FAILURE);
				}
				if (!translation2.valid) {
					printf("Invalid GVA\n");
					fflush(stdout);
					*(uint32_t *)((char *)vm->mem + translation1.physical_address) = 0;
				}
				else{
					uint32_t hva = translation2.physical_address +  (uint32_t)(uintptr_t) vm->mem;
					*(uint32_t *)((char *)vm->mem + translation1.physical_address) = hva;
				}
				numExits++;
				numExitsByIO_out++;
				numExitsByIO_in++;
