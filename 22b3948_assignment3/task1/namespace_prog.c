...
int main() {
    ...
    */
    ...
    // ------------------ WRITE CODE HERE ------------------
    char namespace_path_pid[256];
    snprintf(namespace_path_pid, sizeof(namespace_path_pid),"/proc/%d/ns/pid",child_pid);
    int fd1 = open(namespace_path_pid, O_RDONLY);
    if (fd1 == -1){
        perror("open");
        exit(1);
    }
    ...
    char namespace_path_uts[256];
    snprintf(namespace_path_uts, sizeof(namespace_path_uts),"/proc/%d/ns/uts",child_pid);
    int fd2 = open(namespace_path_uts, O_RDONLY);
    if (fd2 == -1){
        perror("open");
        exit(1);
    }
