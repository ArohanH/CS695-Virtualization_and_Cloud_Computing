...
handle_run() {
    ...
    if [ -d "$CACHEDIR/layers/$layer_hash" ]; then
        ...
        current_layer="$CACHEDIR/layers/$layer_hash" # Arohan: made a change, removed "\diff"
        return
    fi
    ...
    mkdir -p "$CACHEDIR/layers/$layer_hash/merged"

    # Subtask 3.f.2
    # Temporarily mount the overlay filesystem
    lowerdir="$parent_layers"
    upperdir="$CACHEDIR/layers/$layer_hash/diff"
    workdir="$CACHEDIR/layers/$layer_hash/work"
    mergeddir="$CACHEDIR/layers/$layer_hash/merged"


    mount -t overlay overlay -o lowerdir="$lowerdir",upperdir="$upperdir",workdir="$workdir" "$mergeddir" || die "Failed to mount overlayfs in handlerun"

    # Subtask 3.f.3
    # Execute the command in the new mount
    mkdir -p "$mergeddir/dev"
    mount --bind /dev "$mergeddir/dev"  # mount /dev if needed for some commands
    unshare --root "$mergeddir" /bin/bash -c "$command"

    umount "$mergeddir/dev"
...
handle_copy() {
    ...
    mount -t overlay overlay -o lowerdir="$lowerdir",upperdir="$upperdir",workdir="$workdir" "$mergeddir" || die "Failed to mount overlayfs in handlecopy"
    
    # Subtask 3.e.3
    # Copy the files from source to destination
    mkdir -p "$(dirname "$mergeddir/$dest")"
    cp -a "$src" "$mergeddir/$dest"
...
run() {
    ...
    # mkdir -p "$CONTAINERDIR/$NAME"
    LAYER_STACK=$(cat "$IMAGEDIR/$IMAGE/layers")

    UPPERDIR="$CONTAINERDIR/$NAME/upper"
    WORKDIR="$CONTAINERDIR/$NAME/work"
    MERGEDDIR="$CONTAINERDIR/$NAME/merged"

    mkdir -p "$UPPERDIR" "$WORKDIR" "$MERGEDDIR"
    mount -t overlay overlay -o lowerdir="$LAYER_STACK",upperdir="$UPPERDIR",workdir="$WORKDIR" "$MERGEDDIR"
    ...
    mount -o bind /dev "$CONTAINERDIR/$NAME/merged/dev"
    # Subtask 3.a.2
    ...
    # ROOTFS="$CONTAINERDIR/$NAME/rootfs"
    # chmod +755 "$ROOTFS"
    # unshare --uts --pid --net --mount --ipc --kill-child --fork chroot "$ROOTFS" /bin/sh -c "mount -t proc proc /proc; mount -t sysfs sys /sys; exec $INIT_CMD_ARGS"
    # Subtask 3.d.3
    # Modify subtask 3.a.2 to use the overlay filesystem
    ROOTFS="$CONTAINERDIR/$NAME/merged"
    chmod +755 "$ROOTFS"
    unshare --uts --pid --net --mount --ipc --kill-child --fork chroot "$ROOTFS" /bin/sh -c "mount -t proc proc /proc; mount -t sysfs sys /sys; exec $INIT_CMD_ARGS"
}
...
stop() {
    ...
    local PID=$(ps -ef | grep "$CONTAINERDIR/$NAME/merged" | grep -v grep | awk 'NR==1{print $2}')
    ...
    rm -rf "$CONTAINERDIR/$NAME/upper"
    rm -rf "$CONTAINERDIR/$NAME/work"
    rm -rf "$CONTAINERDIR/$NAME/merged"
...
exec() {
    ...
    local UNSHARE_PID=$(ps -ef | grep "$CONTAINERDIR/$NAME/merged" | grep -v grep | awk 'NR==1{print $2}')
    [ -z "$UNSHARE_PID" ] && die "Cannot find container process"
